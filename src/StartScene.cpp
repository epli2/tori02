...
void StartScene::Draw() {
  ...
  if (etime <= 4) {
    ...
    img_tori_.draw(ofGetWidth() / 2 - img_tori_.getWidth() / 2 + x, ofGetHeight() / 2 - img_tori_.getHeight() / 2 + y);
    ofPopStyle();
  }
    ...
    ofPushStyle();
    ofSetColor(255, 255, 255, 255 - 255 * (etime * etime * etime / 64));
    img_title_.draw(ofGetWidth() / 2 - img_title_.getWidth() / 2 + x, ofGetHeight() / 2 - img_title_.getHeight() / 2 + y);
    ofPopStyle();
  }
  if (etime > 8.5) {
    isend_ = true;
